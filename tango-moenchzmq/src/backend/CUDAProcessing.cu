...
__global__ void process_group2D(int *raw_frame, int *class_output_array,
                                float *pedestal_sum,
                                float *pedestal_squared_sum,
                                int *pedestal_counter, bool isPedestal,
                                int *blockIds, int *threadIds) {

  int pixel_rms = 0;
  // for the most left and most right block we need take only one extra pixel
  // from either right or left for the blocks inbetween we need to take both
  // left and right extra overlapping pixels
  ...
  int global_x = (threadIdx.x + gridDim.x * blockIdx.x)
                 - 1; // 20 * [0, 1, 2, 3, ... , 19] -> global x maps 0 ... 399
  int global_y = (threadIdx.y + gridDim.y * blockIdx.y)
                 - 1; // 20 * [0, 1, 2, 3, ... , 19] -> global y maps 0 ... 399
  // top left column is (threadIdx.x == 0 && blockIdx.x == 0)
  // top row is (threadIdx.y == 0 && blockIdx.y == 0)
  // top right column is (threadIdx.x == (block_size - 1) && blockIdx.x ==
  // (grid_size - 1)) bottom row is (threadIdx.y == (block_size - 1) &&
  // blockIdx.y == (grid_size - 1))
  if (blockIdx.x == 0 && threadIdx.x == 0
    ...
    pixel_rms
        = sqrt(pedestal_squared_sum[global_y * 400 + global_x] / pixel_counter
               - pow(pixel_offset, 2));
    no_bkgd_for_block[local_y * block_size + local_x]
        = raw_frame[global_y * 400 + global_x] - pixel_offset;
  }
  __syncthreads();
  ...
  // so we update it from the block where the given pixel is the most right =>
  // where this corresponds to 21th pixel

  int v, tot, tl, tr, bl, br, max_value = 0;
  int pixel_class = 0;
  // considering the global_x we are now inspecting the if a pixel global_x a
  // photon_max -> etc 0 - pedestal, 1- photon,  2 - photon_max, 3 -
  // negative_pedestal
  if (!isPedestal) {
    ...
    if (no_bkgd_for_block[local_y * block_size + local_x]
        < -nsigma * pixel_rms) {
      class_output_array[global_y * 400 + global_x] = 3;
      // considering candidates which are some kind of photons (either partial
      // photon charges or the center )
  ...
  // since we updating only pixels on the right side we ignore the 0th pixel
  // and 21th pixel in the last block if the pixel_class remained unchanged and
  // not associated with any kind of photons it is a pedestal and need to be
  // added to stats
  if (pixel_class == 0) {
    ...
    if (pixel_counter < BUFFER_SIZE) {
      pedestal_sum[global_y * 400 + global_x]
          += raw_frame[global_y * 400 + global_x];
      pedestal_squared_sum[global_y * 400 + global_x]
          += pow(raw_frame[global_y * 400 + global_x], 2);
    } else {
      pedestal_sum[global_y * 400 + global_x]
          += raw_frame[global_y * 400 + global_x]
             - pedestal_sum[global_y * 400 + global_x] / BUFFER_SIZE;
      pedestal_squared_sum[global_y * 400 + global_x]
          += pow(raw_frame[global_y * 400 + global_x], 2)
             - pedestal_squared_sum[global_y * 400 + global_x] / BUFFER_SIZE;
    }
  }
}
...
int main() {
  ...
  cudaMemcpy(input_array_d, test_array_h, IMAGE_SIZE * sizeof(int),
             cudaMemcpyHostToDevice);
  cudaMemcpy(output_array_d, output_array_h, IMAGE_SIZE * sizeof(int),
             cudaMemcpyHostToDevice);
  ...
  // seems like threads per block need to be + (cluster_size / 2) to blocks per
  // grid
  for (int i = 0; i < 200; i++) {
    ...
    process_group2D<<<blocksPerGrid, threadsPerBlock>>>(
        input_array_d, output_array_d, pedestal_sum_d, pedestal_squared_d,
        pedestal_counter_d, true, blockIds_d, threadIds_d);
  }
  cudaMemcpy(pedestal_sum_h, pedestal_sum_d, IMAGE_SIZE * sizeof(float),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(pedestal_squared_h, pedestal_squared_d,
             IMAGE_SIZE * sizeof(float), cudaMemcpyDeviceToHost);
  cudaMemcpy(pedestal_counter_h, pedestal_counter_d, IMAGE_SIZE * sizeof(int),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(blockIds_h, blockIds_d, IMAGE_SIZE * sizeof(int),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(threadIds_h, threadIds_d, IMAGE_SIZE * sizeof(int),
             cudaMemcpyDeviceToHost);

  for (int x = 0; x < 41; x++) {
    printf("pedestal_sum %f, pedestal_sum_sqaured %f, counter %d, blockId %d, "
           "threadId %d\n",
           pedestal_sum_h[x], pedestal_squared_h[x], pedestal_counter_h[x],
           blockIds_h[x], threadIds_h[x]);
  }
  return 0;
}
