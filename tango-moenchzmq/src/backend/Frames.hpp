...
template <typename T, unsigned int V> struct OrderedFrame {
  ...
  T &operator()(int y, int x) { return arr[y * consts::FRAME_WIDTH + x]; }

  template <typename B> void copy_to_buffer(B &buffer, bool flip = true) {
  ...
  operator-(const OrderedFrame<float, consts::LENGTH> &rhs) {
  ...
  void fill(T value) { std::fill(arr, arr + V, value); }
