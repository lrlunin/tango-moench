...
std::filesystem::path HDFWriter::buildFullFilePath() {
  ...
  full_file_path /=
      fmt::format(fmt::runtime(file_format), fmt::arg("file_name", file_name),
...
void HDFWriter::openFile() {
  ...
  unsigned int open_flag =
      (std::filesystem::exists(full_file_path)) ? H5F_ACC_RDWR : H5F_ACC_TRUNC;
...
void HDFWriter::writeFrame(const std::string group_name,
  ...
  const hsize_t image_dimension[2] = {consts::FRAME_HEIGHT,
                                      consts::FRAME_WIDTH};
  ...
  H5::DataSet dataset =
      current_file.createDataSet(image_path, image_datatype, image_dataspace);
...
void HDFWriter::writeFrameStack(const std::string group_name,
  ...
  const hsize_t image_single_dimension[3] = {1, consts::FRAME_HEIGHT,
                                             consts::FRAME_WIDTH};
...
void HDFWriter::writeFrameStack(const std::string group_name,
  ...
  std::string image_path = fmt::format("{}/{}", group_name, frame_stack_name);
  H5::DataSet dataset =
      h5_file.createDataSet(image_path, image_datatype, image_stack_dataspace);
  dataset.write(frame_stack_ptr, image_datatype, image_stack_dataspace);
};
