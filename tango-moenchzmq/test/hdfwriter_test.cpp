...
TEST_F(HDFWriterTest, FileCreation){
    ...
    for (unsigned int x = 0; x < files_to_create; x++){
        ...
        std::filesystem::path full_file_path = "/tmp/" + folder_name + "/" + file_name;
        EXPECT_TRUE(std::filesystem::is_regular_file(full_file_path)) << "File " << full_file_path << " not found";
    }
    // check if the correct number of files is created
    auto files_in_folder = std::distance(std::filesystem::directory_iterator("/tmp/" + folder_name), std::filesystem::directory_iterator{});
...
TEST_F(HDFWriterTest, FileWriteFrame){
    ...
    H5::H5File file("/tmp/" + folder_name + "/" + fmt::format("{:%Y%m%d}_run_{:06d}.h5", now, file_writer_ptr->file_index), H5F_ACC_RDONLY);
...
TEST_F(HDFWriterTest, FileWriteFrameStack){
    ...
    // write frame to file
    file_writer_ptr->openFile();
    file_writer_ptr->writeFrameStack("group", "frame_stack", frame_stack, frame_stack_length);
    file_writer_ptr->closeFile();
